  ...
  return (
    <header className="bg-white border-b border-pink-100 sticky top-0 z-50">
      <div className="container mx-auto px-2 py-2 flex items-center">
        {/* 로고 */}
        <Link href="/" className="text-3xl font-bold text-pink-500 font-serif tracking-tight lg:hidden shrink-0">
          DaCare
        </Link>
        <Link href="/" className="hidden lg:flex flex-col items-start leading-tight shrink-0">
        ...
        <nav className="hidden lg:flex flex-1 flex-col items-center gap-2 py-1">
          ...
          <ul className="flex items-center space-x-5">
            {navLinks.map(link => (
              <li key={link.href}>
                <Link href={link.href} className={`flex items-center transition-colors font-medium whitespace-nowrap ${
                  link.href === '/admin' ? 'text-blue-500 hover:text-blue-700' : 'text-gray-600 hover:text-pink-500'
                }`}>
                  <link.icon className="mr-1.5" size={15} /> {link.label}
                </Link>
              </li>
            ))}
          </ul>
